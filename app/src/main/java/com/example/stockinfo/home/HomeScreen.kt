...
import com.example.stockinfo.stockInfoTabRowScreens
...
fun HomeScreen(modifier: Modifier = Modifier,
    ...
    var removeIsClicked = remember {mutableStateOf(false)}
    var removeStockID = remember { mutableStateOf(0L) }
    ...
    {
        StockInfoList(
            stockInfoList = homeUiState.stockInfoList,
            onItemClick = {item->
                removeStockID.value = item.stockID
                removeIsClicked.value = true
            },
            contentPadding = PaddingValues(5.dp)
        )
        if(removeIsClicked.value == true){
            removeStockInfoDialog(
                removeStockID.value,
                onDismissRequest = {removeIsClicked.value = false},
                onConfirmation = {it->
                    homeScreenViewModel.removeStockInfoByStockID(removeStockID.value,)
                }
            )
        }
        ...
        {
            Icon(
                imageVector = Icons.Default.AddCircle, // Use a Material Design icon
                contentDescription = "Add stock into Favorite List" // Provide content description for accessibility
            )
        }
        if (isClicked.value) {
            Log.d("DEBUG", "1  isClicked.value = " + "${isClicked.value}")
            //處理點選新增callback
            AddStockInfoDialog(
                onConfirmation = {
                    Log.d("DEBUG", "input  ${it}  GGG")
                    Log.d("DEBUG", "GGG")
                    if (it.isEmpty()){
                        Log.d("DEBUG", "con1  GGG")
                        return@AddStockInfoDialog
                    }

                    val stockID = it.split(" ")[0].toLong()
                    val stockName = it.split(" ")[1]
                    if(
                        homeScreenViewModel.checkStockInfoExistByStockID(
                            homeUiState.stockInfoList,
                            stockID
                        )
                    ){
                        Log.d("DEBUG", "con2 GGG")
                        //股票代號已經存在我的最愛
                        return@AddStockInfoDialog
                    }
                    else{
                        Log.d("DEBUG", "con3  GGG")
                        //存入股票代號到我的最愛
                        homeScreenViewModel.addStockInfoByOne(
                            DialogUiState(StockInfo(0, stockID, stockName))
                        )
                    }
                },
                onDismissRequest = {
            ...
            Log.d("DEBUG", "2  isClicked.value = " + "${isClicked.value}")
        }
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun DemoHomeScreen(modifier: Modifier = Modifier,
               homeScreenViewModel: HomeScreenViewModel
               = viewModel(factory = AppViewModelProvider.Factory),
               onLongClickMyfavoriteList: () -> Unit = {},
){
    Log.d("HomeScreen", "HomeScreen()")
    //val homeUiState by homeScreenViewModel.homeUiState.collectAsState()
    val data1 = StockInfo(stockID = 1101, stockName = "台灣水泥")
    val data2 = StockInfo(stockID = 2330, stockName = "台Ｇ電")
    val data3 = StockInfo(stockID = 2885, stockName = "元大金")


    val stockinfolist = listOf(data1, data2, data3)
